...
function main(workbook: ExcelScript.Workbook){
     ...
     */
    employees.validate_key(standard_bonuses, "Title", "raise");
    ...
    function confidence_interval(row: fr.Row): string {
        let avg = Number(row["Salary_mean"]);
        let std_dev = Number(row["Salary_std_dev"]);
        let count = Number(row["Salary_count"]);
        const Z = 1.96;
    ...
    let avg_salaries = employees.groupBy("Department", ["Salary", "Salary"], ["mean", "std_dev"]);
